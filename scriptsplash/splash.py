...
align_options = ['center', 'middle', 'left', 'right']
...
class Canvas:
    ...
    def horizontal_line(self, is_filled=True):
        if is_filled:
            ...
            return (
                self.horizontal.text
                + ' ' * (self.width - 2)
                + self.horizontalh.text
            )
    ...
    def separator(self, symbol=None, color=None):
        ...
        if symbol is not None:
            return (
                self.vertical.text
                + symbol_string.text * (self.width - 2)
                + self.vertical.text
            )
        else:
            return (
                self.center_left.text
                + self.horizontal.text * (self.width - 2)
                + self.center_right.text
            )
    ...
    def splash_cpp(self):
        ...
        result += (
            '// check https://github.com/aureliencarle/scriptsplash.git \n'
        )
        result += 'std::string splashFromPythonToCpp()\n'
        result += '{\n'
        result += (
            '    std::string splash = "'
            + repr(self.generate_string())[1:-1]
            + '";\n'
        )
...
class Splash(Canvas):
    ...
    def add_ansi_logo(
        self, logo, font='ANSI Shadow', align='left', color=None
    ):
        self.add_multi_line(
            self.generate_ansi(logo, font, color=color), align=align
        )
    ...
    def add_one_content_line(
        self, line, align='left', color=None, background=None
    ):
        if self.global_color is not None:
            color = self.global_color
    ...
    def add_two_content_line(
        self, line1, line2, align='left', color1=None, color2=None
    ):
        if self.global_color is not None:
